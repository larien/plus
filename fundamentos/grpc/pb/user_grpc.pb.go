...
type UserServiceClient interface {
	...
	AddUserBi(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserBiClient, error)
}
...
func (c *userServiceClient) AddUserBi(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserBiClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUserBi", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUserBiClient{stream}
	return x, nil
}

type UserService_AddUserBiClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUserBiClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUserBiClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUserBiClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUserBi(UserService_AddUserBiServer) error
...
func (UnimplementedUserServiceServer) AddUserBi(UserService_AddUserBiServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUserBi not implemented")
}
...
func _UserService_AddUserBi_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUserBi(&userServiceAddUserBiServer{stream})
}

type UserService_AddUserBiServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUserBiServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUserBiServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUserBiServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUserBi",
			Handler:       _UserService_AddUserBi_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
